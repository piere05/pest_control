...
<h5 class="mb-0 text-dark">AMC Remainder</h5>
</div>
<hr/>
...
<? 
$select_job_order=mysqli_query($conn,"select * from job_order job where type_of_job=1 and job_date=(select max(job_date) from job_order where quotation_id=job.quotation_id and type_of_job=1) $subqry order by id desc");
...
<td><p class="mb-0 text-danger fw-600"><?=$job_date;?></p></td>
<td><?=$job_time;?></td>
