...
if ($search_technician_id!='') {
$subqry.="and Id='$search_technician_id'";
}
...
<h5 class="mb-0 text-dark">Technician Report</h5>
</div>
<hr/>
...
<div class="col-md-3">
<label for="inputFirstName" class="form-label">Select Technician</label>
<select class="single-select" name="search_technician_id" id="search_technician_id">
<option value="">Select Technician</option> 

<?
$select_supervisor=mysqli_query($conn,"select * from user where user_type='3'");

while ($row_customer=mysqli_fetch_array($select_supervisor)) {

   $technician_name=$row_customer['name'];
   $mobile_no=$row_customer['UserName'];
   $company_name=$row_customer['company_name'];
   $supervisor_id=$row_customer['Id'];
?>
<option value="<?=$supervisor_id?>" <?if($search_technician_id==$supervisor_id){echo "selected";}?>><?=$technician_name." - ".$mobile_no;?></option>
<?
}
?>
</select>
...
<? $select_report=mysqli_query($conn,"select * from user where 1=1 and user_type=3 $subqry order by id desc");
...
$SNo = $SNo + 1; 
$id=$row_report['Id'];
$Technician_mobile=$row_report['UserName'];
$Technician_name=$row_report['name'];


$select_job_order=mysqli_query($conn,"select * from technician_log where technician_id='$id'");

$total_minutes = 0;
while($row_technician_log=mysqli_fetch_array($select_job_order)){
 $clock_in_time  = strtotime($row_technician_log['clock_in_time']);
 $clock_out_time = strtotime($row_technician_log['clock_out_time']);

if ($clock_in_time !='' && $clock_out_time!='') {
 $diff_minutes = ($clock_out_time - $clock_in_time) / 60;
...
<td>+971 <?= $Technician_mobile; ?> </td>

<td><?=mysqli_num_rows($select_job_order); ?></td>

<td><?=$total_hours.$Minutes; ?> hrs</td>
